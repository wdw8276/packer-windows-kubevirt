...
variable "autounattend" {
  type    = string
  default = "./answer_files/2022_datacenter_kubevirt/Autounattend.xml"
}
...
variable "iso_url" {
  type    = string
  default = "./iso/en_windows_server_2022x64_dvd_.iso"
}
...
variable "vm_name" {
  type    = string
  default = "windows_2022_datacenter_kubevirt"
}
...
source "qemu" "win2022_datacenter_kubevirt" {
  iso_url      = var.iso_url
...
build {
  sources = ["source.qemu.win2022_datacenter_kubevirt"]
  ...
  provisioner "powershell" {
    scripts = ["./scripts/init-kubevirt-win2022.ps1"]
  }

  provisioner "windows-shell" {
    execute_command = "{{ .Vars }} cmd /c C:/Windows/Temp/script.bat"
    remote_path     = "c:/Windows/Temp/script.bat"
    scripts         = ["./scripts/85-disable-bitlocker.bat", "./scripts/90-compact.bat", "./scripts/95-pre-sysprep-win2022.bat"]
  }
